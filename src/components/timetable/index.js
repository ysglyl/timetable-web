            ...
            <Fragment key={`fragment_${row}`}>
              {
                new Array(columns).fill(0).map((_, column) => (
                    <Card.Grid key={`item_${row}_${column}`} hoverable={this.hoverableTableItem(row, column)}
                               style={{...gridStyle, textAlign: 'center'}}>
                      <div style={{width: '100%', height: '100%'}} onClick={() => this.clickTableItem(row, column)}>
              ...
              {dividers.includes(row) &&
              <Card.Grid key={`divider_${row}`} hoverable={false} style={gridDividerStyle} />}
            </Fragment>)
          )}
        </Card>
      </div>
